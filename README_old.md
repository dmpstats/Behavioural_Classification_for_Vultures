...
<!--  This MoveApp generates speed-as-time-of-day models to be used to generate confidence and prediction intervals and identify abnormally high speeds or long durations of stationary behaviour.

The data is split by individual and time-based attributes are appended for model fitting. A Poisson model is fitted to that animal's speed as a function of hour and minute. A generalized additive model (GAM) is then applied using runSALSA1D from the MRSea package. The input data is separated from the model objects, which are compiled into a list and released as a .rds artifact within a .zip file.

It is recommended that you do not use this MoveApp on large volumes of data or data containing a large number of individuals. Consider filtering your data before providing it as an input. However, note that the models cannot be fitted to individuals with fewer than 10 associated events.

The 'data' element of the outputted models are removed for storage purposes.

- Make sure that, within the input dataset, each individual has more than 10 associated locations. Any individuals with insufficient data will not have a model generated on their data

- High volumes of data will cause this MoveApp to run for excessively long durations. We recommend filtering the data to a smaller interval (minimum 5-minute intervals over a period of less than 1 year) before applying this method
