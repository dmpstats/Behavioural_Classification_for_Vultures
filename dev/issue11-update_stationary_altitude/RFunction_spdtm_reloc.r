...
rFunction_spdtm_reloc = function(data, 
                     travelcut,
                     create_plots = TRUE,
                     sunrise_leeway = 0,
                     sunset_leeway = 0,
  ...
  ### [6.1] Stationary Speed Vs day-hours model  -----------------
  ...
  ### [6.2] apply Speed Vs day-hours Rule  -----------------
  data %<>% 
    ungroup() %>%
    mutate(
  ...
  if (keepAllCols == FALSE) {
    ...
    data %<>% dplyr::select(-any_of(
      c(
        "ID", "endofday_dist_m", "roostsite", "travel01", "cum_trav", "revcumtrav", "stationaryNotRoost", "cumtimestat"
      ) 
    ))
  }
...
#' /////////////////////////////////////////////////////////////////////////////////////////////
speed_time_model_new <- function(dt, p){
