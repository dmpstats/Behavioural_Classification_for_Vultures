...
rFunction_altitude_daytime = function(data, 
                         travelcut,
                         create_plots = TRUE,
                         sunrise_leeway = 0,
                         sunset_leeway = 0,
  ...
  progressr::with_progress({
    ...
    data <- data |> 
      ...
      dplyr::group_split() |> 
      furrr::future_map(
        .f = speed_time_model_old, travelcut = travelcut, p = p,
  ...
  if(alt_classify){
    ...
    data %<>%
      ...
      mutate(
        RULE = case_when(
          (nightpoint == 0) & (behav == "SResting") & (altchange == "ascent") ~ "[2] Altitude increasing",
          (nightpoint == 0) & (behav == "SResting") & (altchange == "descent") & (lead(altchange) %in% c("descent", "ascent")) ~ "[2] Altitude decreasing",
          TRUE ~ RULE
        ),
        behav = case_when(
          (nightpoint == 0) & (behav == "SResting") & (altchange == "ascent") ~ "STravelling",
          (nightpoint == 0) & (behav == "SResting") & (altchange == "descent") & (lead(altchange) %in% c("descent", "ascent")) ~ "STravelling",
          TRUE ~ behav
        ),
...
#' /////////////////////////////////////////////////////////////////////////////////////////////
speed_time_model_old <- function(dt, travelcut, p){
